...
/**
 *	JPIERE-0193:Generate PO from Sales Order only Dropship Product
 *
 *  @author Hideaki Hagiwara
 *
 *  Please refer OrderPOCreate.java
 *
 */
public class OrderPOCreateOnlyDropshipProduct extends SvrProcess
{
	/**	Order Date From		*/
	...
	private int createPOFromSO (MOrder so) throws Exception
	{
		...
		String sql = "SELECT MIN(po.C_BPartner_ID), po.M_Product_ID "
			...
			+ " INNER JOIN C_OrderLine ol ON (po.M_Product_ID=ol.M_Product_ID AND ol.Link_OrderLine_ID IS NULL) "
			+ " INNER JOIN M_Product p ON (ol.M_Product_ID=p.M_Product_ID) "
			+ "WHERE ol.C_Order_ID=? AND po.IsCurrentVendor='Y' AND P.IsDropShip='Y' "
			+ "AND po.IsActive='Y' "
