...
public class AccessUpdateUserRole extends SvrProcess {
	...
	private void doActive()
	{
		...
		if(p_AD_Role_ID == 0)
		{
			...
			activeNum = DB.executeUpdate(sql.toString(), new Object[] {now,now,getAD_Client_ID(),p_AD_Role_ID},false, get_TrxName(),0);
		}
	}
	...
	private void doInActive()
	{
		...
		if(p_AD_Role_ID == 0)
		{
			...
			sql = sql.append(" AND AD_Role_ID = ?"); 
			inActiveNum = DB.executeUpdate(sql.toString(), new Object[] {now,now,getAD_Client_ID(),p_AD_Role_ID},false, get_TrxName(),0);
		}
