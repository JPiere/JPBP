...
public class UpdateInventoryValuationPriceList extends SvrProcess {
	...
	private int		p_JP_InvValCal_ID = 0;
	...
	protected void prepare()
	{
		...
		p_JP_InvValCal_ID = getRecord_ID();

		if(p_JP_InvValCal_ID > 0)//Kicked From Inv Val Cal Doc
		{
			m_InvValCal = new MInvValCal(getCtx(), p_JP_InvValCal_ID, get_TrxName());
			p_JP_InvValProfile_ID = m_InvValCal.getJP_InvValProfile_ID();
			p_DateValue = m_InvValCal.getDateValue();

		}else { //Kicked From Process parameter Panel
	...
	protected String doIt() throws Exception {
		...
		if(ivp.getCostingLevel().equals(MInvValProfile.COSTINGLEVEL_Client))
		{
			...
			if(m_InvValCal == null)
				createProductPriceFromCost(ivp, 0, version);
			else
			...
			for(int i = 0; i < orgs.length; i++)
			{
				...
				if(m_InvValCal == null)
					createProductPriceFromCost(ivp, orgs[i].getAD_Org_ID(), version);
				else
	...
	private void createProductPrice(MInvValProfile ivp, int AD_Org_ID, MPriceListVersion version) throws Exception
	{
		...
		MInvValCalLine[]  m_InvValCalLines =  m_InvValCal.getLines();
		MProductPrice pp = null;

		for(int i = 0 ; i < m_InvValCalLines.length; i++)
		{
			pp = new MProductPrice(getCtx(), version.getM_PriceList_Version_ID(), m_InvValCalLines[i].getM_Product_ID(), get_TrxName());
			pp.setAD_Org_ID(version.getAD_Org_ID());
			pp.setPriceList(m_InvValCalLines[i].getJP_InvValAmt());
			pp.setPriceStd(m_InvValCalLines[i].getJP_InvValAmt());
			pp.setPriceLimit(m_InvValCalLines[i].getCurrentCostPrice());
			pp.saveEx(get_TrxName());
		}

	}

	private void createProductPriceFromCost(MInvValProfile ivp, int AD_Org_ID, MPriceListVersion version) throws Exception
	{
		int M_Product_ID = 0;
