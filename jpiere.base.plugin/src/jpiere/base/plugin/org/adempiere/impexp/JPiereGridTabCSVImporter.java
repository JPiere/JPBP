 ...
 */
public class JPiereGridTabCSVImporter implements IGridTabImporter
{
		...
		try {
			...
			if ( !m_isError ) {
				...
				for (int idx = 0; idx < data.size(); idx++) {
					...
					if (trx != null)
						trx.setDisplayName(JPiereGridTabCSVImporter.class.getName()+"_fileImport_" + gridTab.getTableName());//JPIERE
	...
	private CellProcessor getProccesorFromColumn(GridField field) {
		...
		if (DisplayType.Date == field.getDisplayType()) {	//JPIERE-0462
			String formatPattern = field.getFormatPattern();
			if(!Util.isEmpty(formatPattern))
			{
				return (new Optional(new ParseDate(formatPattern)));
			}else {
				return (new Optional(new ParseDate(DisplayType.DEFAULT_DATE_FORMAT)));
			}
		} else if (DisplayType.DateTime == field.getDisplayType()) {//JPIERE-0462
			String formatPattern = field.getFormatPattern();
			if(!Util.isEmpty(formatPattern))
			{
				return (new Optional(new ParseDate(formatPattern)));
		...
		} else if (DisplayType.Time == field.getDisplayType()) {//JPIERE-0462
			String formatPattern = field.getFormatPattern();
			if(!Util.isEmpty(formatPattern))
			{
				return (new Optional(new ParseDate(formatPattern)));
