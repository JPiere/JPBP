 ...
 */
public class JPiereProductPricing extends MProductPricing
{
	 ...
	 */
	public JPiereProductPricing (int M_Product_ID, int C_BPartner_ID,
	...
	public void setOrderLine(I_C_OrderLine orderLine, String trxName)
	{
		...
		if(orderLine.getC_Order() != null)
		{
			...
			GridTab gt = gtw.getGridTab();
			m_isSOTrx = Env.getContext(Env.getCtx(),gt.getWindowNo(), "IsSOTrx").equals("Y");
		}
	...
	public void setInvoiceLine(I_C_InvoiceLine invoiceLine, String trxName) {
		...
		if(invoiceLine.getC_Invoice() != null)
		{
			...
			GridTabWrapper gtw = (GridTabWrapper)java.lang.reflect.Proxy.getInvocationHandler(invoiceLine);
			GridTab gt = gtw.getGridTab();

			if(gt.getTableName().equals(MRecognitionLine.Table_Name))
			{
				m_C_BPartner_ID = Env.getContextAsInt(Env.getCtx(), gt.getWindowNo(), gt.getParentTab().getTabNo() , "C_BPartner_ID");
				m_isSOTrx = Env.getContext(Env.getCtx(),gt.getWindowNo(), "IsSOTrx").equals("Y");
				m_PriceDate = Env.getContextAsDate(Env.getCtx(),gt.getWindowNo(), "DateInvoiced");

			}else if (gt.getTableName().equals(MContractLine.Table_Name)) {

				m_C_BPartner_ID = Env.getContextAsInt(Env.getCtx(), gt.getWindowNo(), gt.getParentTab().getTabNo() , "C_BPartner_ID");
				m_isSOTrx = Env.getContext(Env.getCtx(), gt.getWindowNo(), gt.getParentTab().getTabNo() , "IsSOTrx").equals("Y");
				m_PriceDate = Env.getContextAsDate(Env.getCtx(),gt.getWindowNo(), "DateDoc");
