			...
			if(po instanceof MBill)
			{
				...
				if(bill.getJP_LastBill_ID() == 0 && bill.getJPLastBillAmt().compareTo(Env.ZERO) == 0)
				{
					String whereClause = " C_BPartner_ID =? AND C_DocType_ID = ? AND AD_Org_ID =? AND C_PaymentTerm_ID = ? "
											+ " AND C_Currency_ID = ? AND DocStatus in ('CO','CL') AND JPDateBilled < ? ";
					...
					lastBill = new Query(po.getCtx(), MBill.Table_Name, whereClause, po.get_TrxName())
							.setParameters(bill.getC_BPartner_ID(), bill.getC_DocType_ID(), bill.getAD_Org_ID(), bill.getC_PaymentTerm_ID(), bill.getC_Currency_ID(), bill.getJPDateBilled())
				...
				{
					String whereClause = " JP_Bill_ID =? AND DocStatus in ('CO','CL') ";
					...
					lastPayment = new Query(po.getCtx(), MPayment.Table_Name, whereClause, po.get_TrxName())
							.setParameters(lastBill.getJP_Bill_ID())
							.setOrderBy(orderClause)
							.first();
				...
				{
					String sql = "SELECT bs.C_BankAccount_ID FROM JP_BillSchema bs INNER JOIN C_BPartner bp ON (bs.JP_BillSchema_ID = bp.JP_BillSchema_ID)"
