...
public class JPiereAttachmentUndeletableModelValidator implements ModelValidator {
	...
	private int undeletableSQL(PO po)
	{
		String sql = "UPDATE JP_AttachmentFileRecord i"
				+ " SET IsDeleteable= 'N' WHERE AD_Table_ID = ? AND Record_ID= ?";

		Object[] tableRecord_ID = {po.get_Table_ID(), po.get_ID()};

		return DB.executeUpdate(sql,tableRecord_ID,true, po.get_TrxName());
	}
