...
/**
 * Post Inventory Valuation Adjust Documents.
 *
 *
 * JPIERE-0163:JPBP:InvValCal
 *
 * <pre>
 *   Table:              JP_InvValAdjust
 *   Document Types:     JPA
 * </pre>
 *
 * @author Jorg Janke
 ...
 * @version α
 */
public class Doc_JPInvValAdjust extends Doc
{
	/**
	 * Constructor
	 ...
	 * 	@param trxName trx
	 */
	public Doc_JPInvValAdjust (MAcctSchema as, ResultSet rs, String trxName)
	{
		super (as, MInvValAdjust.class, rs, DOCTYPE_PurchaseRequisition, trxName);
	}
	...
	protected String loadDocumentDetails ()
	{
		...
		MInvValAdjust iva = (MInvValAdjust)getPO();
		setDateDoc (iva.getDateAcct());
		setDateAcct (iva.getDateAcct());
		// Amounts
		setAmount(AMTTYPE_Gross, iva.getTotalLines());
		setAmount(AMTTYPE_Net, iva.getTotalLines());
		// Contained Objects
		p_lines = loadLines (iva);
	 ...
	 */
	private DocLine[] loadLines (MInvValAdjust iva)
	{
		ArrayList<DocLine> list = new ArrayList<DocLine> ();
		MInvValAdjustLine[] lines = iva.getLines();
		for (int i = 0; i < lines.length; i++)
		{
			MInvValAdjustLine line = lines[i];
